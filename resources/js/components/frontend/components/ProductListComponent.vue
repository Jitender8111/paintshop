<template>
    <div v-if="products.length > 0" v-for="product in products"
        ...
        <div class="px-1 sm:px-0 pt-4 pb-2">
            ...
            <div class="flex items-center gap-2 flex-wrap" v-if="product.is_offer">
            <h3 class="text-xl sm:text-[22px] font-bold text-black">
                {{ product.discounted_price }}
            </h3>
            <h4 class="text-sm sm:text-base font-semibold text-gray-500 line-through">
            ...
            <h4 class="text-xl sm:text-[22px] font-bold text-black">
                {{ product.currency_price }}
            </h4>
            <span v-if="product.discount && product.discount > 0"
                class="bg-red-600 text-white text-xs font-bold py-1 px-2 rounded-full shadow">
            ...
            Refundable:
            <span :class="product.refundable === 'Yes' ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'">
                {{ product.refundable }}
            </span>
            </p>
        </div>
