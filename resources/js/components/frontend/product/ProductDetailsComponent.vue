        ...
        <div class="container">
         ...
                    </h4>
                    ...
                    <h3 class="flex items-start gap-4 mb-5">
                        ...
                        <span v-if="product.discount_percentage && product.discount_percentage > 0"
                            class="bg-red-600 text-white text-xs font-bold py-1 px-2 rounded-full shadow">
                            -{{ product.discount_percentage }}%
                        </span>
                    </h3>

                    <!-- 🧾 Refundable Flag -->
                    <p v-if="product.refundable == 'No'" class="text-red-500 font-semibold mb-4">
                        ⚠️ This product is not refundable.
                    </p>
                    <p v-else class="text-green-600 font-semibold mb-4">
                    ...
                    <dl class="flex flex-wrap items-center gap-x-6 gap-y-3 mb-8">
                        ...
                        <dd class="flex items-center gap-6">
                            ...
                            <div v-if="!initialVariations.length || selectedVariation != null">
                                ...
                                <!-- ⚠️ Low Stock Warning -->
                                <p
                                v-else-if="lowStockWarning && temp.stock > 0"
                                class="text-red-600 font-semibold"
                                >
                                ⚠️ Hurry! Only {{ temp.stock }} item<span v-if="temp.stock > 1">s</span> left in stock.
                                </p>
                    ...
                    <div class="flex flex-wrap items-center gap-8 mb-5">
                         ...
                         <div>
                            ...
                            >
                                ...
                                <span class="whitespace-nowrap font-bold">Go To Cart</span>
                            </button>

                            <!-- If product does NOT exist in cart -->
                            <button v-else  @click.prevent="addToCart" :disabled="enableAddToCardButton" type="button"
                        ...
                        <button type="button" @click="wishlist(product.wishlist = !product.wishlist)"
                            ...
                            class="flex items-center justify-center w-12 h-12 rounded-full bg-white shadow-btn-secondary transition-all duration-300 hover:scale-110">
                            <i :class="product.wishlist ? 'lab-fill-heart' : 'lab-line-heart'" class="text-xl"></i>
                        </button>

                        <!-- 📞 Contact -->
                        <router-link to="/page/contact-us"
                            class="flex items-center justify-center w-12 h-12 rounded-full bg-white shadow-btn-secondary text-green-600 transition-all duration-300 hover:scale-110">
                            <i class="fa fa-phone text-xl"></i>
                        </router-link>

                        <!-- 🔗 Share -->
                        <button @click="shareProduct" type="button"
...
export default {
    ...
    data() {
        return {
            ...
            // 🟢 added variables for stock visibility & warning
            stockFlag: true,
    ...
    computed: {
        ...
        // 🟢 added computed for low stock warning
        lowStockWarning() {
            return (
                this.temp.stock > 0 &&
                this.low_stock_quantity_warning > 0 &&
                this.temp.stock <= this.low_stock_quantity_warning
            );
        },
    },
    mounted() {
        ...
        show: function () {
            if (typeof this.$route.params.slug !== "undefined") {
                ...
                this.$store.dispatch("frontendProduct/show", this.props.search).then((res) => {
                    // 🟢 get backend values
                    this.stockFlag = res.data.data.show_stock_out; 
                    this.low_stock_quantity_warning = res.data.data.low_stock_quantity_warning ?? 0;
